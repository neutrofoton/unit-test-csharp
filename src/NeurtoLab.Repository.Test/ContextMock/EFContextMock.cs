...
using NeurtoLab.Repository.Test.Async;

namespace NeurtoLab.Repository.Test.ContextMock;
public class EFContextMock
{
         ...
         where TEntity : BaseModel<TId> 
    {
        ...
        dbSetMock.As<IAsyncEnumerable<TEntity>>()
                ...
                .Returns(new CustomAsyncEnumerator<TEntity>(queryable.GetEnumerator()));
        dbSetMock.As<IQueryable<TEntity>>()
            .Setup(m => m.Provider)
            .Returns(new CustomAsyncQueryProvider<TEntity>(queryable.Provider));
        dbSetMock.As<IQueryable<TEntity>>()
        ...
        dbSetMock.Setup(x => x.Add(It.IsAny<TEntity>())).Callback<TEntity>(data.Add);
        dbSetMock.Setup(x => x.AddRange(It.IsAny<IEnumerable<TEntity>>())).Callback<IEnumerable<TEntity>>(data.AddRange);

        dbSetMock.Setup(x => x.Remove(It.IsAny<TEntity>())).Callback<TEntity>(t => data.Remove(t));
        ...
        dbSetMock.Setup(x => x.Update(It.IsAny<TEntity>())).Callback<TEntity>(t =>
        {
        ...
        dbSetMock.Setup(x => x.UpdateRange(It.IsAny<IEnumerable<TEntity>>())).Callback<IEnumerable<TEntity>>(ts =>
        {
            foreach (var t in ts) { 
